...
const auditSelect = "public_id, user_id, action, entity_type, entity_id, payload, created_at"
...
func (r *Repository) ListAudit(ctx context.Context, where []string, args []any, limit, offset int) ([]map[string]any, int64, error) {
	total, err := r.Count(ctx, fmt.Sprintf(models.QueryGenericListCountFormat, "audit_logs", strings.Join(where, " AND ")), args...)
	if err != nil {
		return nil, 0, err
	}
	args = append(args, limit, offset)
	items, err := r.QueryMaps(ctx, fmt.Sprintf(models.QueryGenericListFormat, auditSelect, "audit_logs", strings.Join(where, " AND "), len(args)-1, len(args)), args...)
