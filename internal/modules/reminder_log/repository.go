...
const reminderLogSelect = "public_id, member_id, subscription_id, reminder_rule_id, channel, recipient, status, sent_at, provider_message_id, error_message, created_at, updated_at"
...
func (r *Repository) ListLogs(ctx context.Context, where []string, args []any, limit, offset int) ([]map[string]any, int64, error) {
	total, err := r.Count(ctx, fmt.Sprintf(models.QueryGenericListCountFormat, "reminder_logs", strings.Join(where, " AND ")), args...)
	if err != nil {
		return nil, 0, err
	}
	args = append(args, limit, offset)
	items, err := r.QueryMaps(ctx, fmt.Sprintf(models.QueryGenericListFormat, reminderLogSelect, "reminder_logs", strings.Join(where, " AND "), len(args)-1, len(args)), args...)
